...
export const ContabilidadAuth = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        if(decoded.role != "superadmon" && decoded.role !== "admin" && decoded.role !== "contabilidad") throw new ResponseError(401, "No tienes permisos");
